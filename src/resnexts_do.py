...
def ResNext_do(blocks, block, cardinality=32, input_tensor=None, input_shape=None, include_top=True, net_type=NetType.vanilla, do_p=0.3,
    ...
    :param cardinality: The number of paths inside the ResNeXt block
    ...
    for stage_id, iterations in enumerate(blocks):
        for block_id in range(iterations):
            for path_id in range(cardinality):
                x = block(features, stage_id, block_id, cardinality=cardinality, do_p=do_p, net_type=net_type, numerical_name=(block_id > 0 and numerical_names[stage_id]))(x)
...
def ResNet18(inputs, blocks=None, include_top=True, classes=1000, *args, **kwargs):
    ...
    return ResNext_do(inputs, blocks, block=keras_resnet.blocks.basic_2d, include_top=include_top, classes=classes, *args, **kwargs)
...
def ResNext50(inputs, blocks=None, cardinality=32, include_top=True, classes=1000, *args, **kwargs):
    """
    ...
    Usage:
        ...
        >>> model = keras_resnet.models.ResNext50(x)
    ...
    return ResNext_do(blocks, cardinality=cardinality, input_tensor=inputs, numerical_names=numerical_names,
                      block=bottleneck_2d_keras_resnext_do, include_top=include_top, classes=classes, #net_type=net_type, do_p=do_p, classes=classes,
                      *args, **kwargs)


def ResNext101(inputs, blocks=None, include_top=True, classes=1000, *args, **kwargs):
    """
    ...
    return ResNext_do(inputs, blocks, numerical_names=numerical_names, block=bottleneck_2d_keras_resnext_do, include_top=include_top, classes=classes, *args, **kwargs)
...
def ResNet152(weights, input_tensor=None, input_shape=None, blocks=None, include_top=True, classes=1000,
    ...
    model = ResNext_do(blocks, input_tensor=input_tensor, input_shape=input_shape, numerical_names=numerical_names,
...
def ResNet200(inputs, blocks=None, include_top=True, classes=1000, *args, **kwargs):
    ...
    return ResNext_do(inputs, blocks, numerical_names=numerical_names, block=bottleneck_2d_do, include_top=include_top,
...
def bottleneck_2d_do(filters, stage=0, block=0, path=0, kernel_size=3, do_p=0.3, net_type=NetType.mc, numerical_name=False, stride=None):
    """
    A two-dimensional bottleneck block.
    ...
    :param path: int representing this path in block (starting from 0)
    ...
    def f(x):
        ...
        y = keras.layers.Conv2D(filters * 2, (1, 1), use_bias=False, name="res{}{}_branch2c".format(stage_char, block_char), **parameters)(y)
        y = keras.layers.BatchNormalization(axis=axis, epsilon=1e-5, name="bn{}{}_branch2c".format(stage_char, block_char))(y)
        if net_type == NetType.mc:
            y = keras.layers.Dropout(do_p)(y, training=True)
        elif net_type == NetType.mc_df:
...
def bottleneck_2d_keras_resnext_do(filters, stage=0, block=0, path=0, cardinality=32, kernel_size=3, do_p=0.3, net_type=NetType.mc, numerical_name=False, stride=None):
    """
    A two-dimensional bottleneck block.
    ...
    def f(x):
        ...
        y = keras.layers.ZeroPadding2D(padding=1, name="padding{}{}_branch2b".format(stage_char, block_char))(y)
        y = keras.layers.DepthwiseConv2D(kernel_size, strides=(1,1), depth_multiplier=c, use_bias=False, name="res{}{}_branch2b".format(stage_char, block_char), **parameters)(y)
        y_shape = keras.backend.int_shape(y)[1:-1]
        y = keras.layers.Reshape(y_shape + (cardinality, c, c))(y)
        y = keras.layers.Lambda(
            lambda x: sum(x[:, :, :, :, i] for i in range(c)),
            name="res{}{}_branch2b_reduce".format(stage_char, block_char))(y)
        y = keras.layers.Reshape(y_shape + (filters,))(y)
        ...
        if block == 0:
            shortcut = keras.layers.Conv2D((64 // cardinality) * filters, (1, 1), strides=stride, use_bias=False, name="res{}{}_branch1".format(stage_char, block_char), **parameters)(x)
            shortcut = keras.layers.BatchNormalization(axis=axis, epsilon=1e-5, name="bn{}{}_branch1".format(stage_char, block_char))(shortcut)
            if net_type == NetType.mc:
                #print('MC____________')
                shortcut = keras.layers.Dropout(do_p)(shortcut, training=True)
