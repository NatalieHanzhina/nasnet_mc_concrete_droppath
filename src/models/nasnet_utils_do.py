...
class ScheduledDropout(Layer):
    ...
    def call(self, inputs, training=None):
        ...
        def dropped_inputs():
            ...
            tf.debugging.assert_equal(tf.reduce_sum(binary_tensor), tf.reduce_sum(tf.cast(tf.reduce_max(outputs, axis=[1,2,3]) > 0, dtype=tf.float32)))  # TODO: remove when debugged
